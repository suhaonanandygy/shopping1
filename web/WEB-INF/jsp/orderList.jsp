    ...
    <div class="span24">
        ...
        <table>
            ...
            <c:forEach items="${pageBean.list }" var="order">
                <tr>
                    <th colspan="5">订单编号:<font color="red">${order.order_id}</font>&nbsp;&nbsp;&nbsp;&nbsp;
                        订单金额:<font color="red">${order.order_totalprice}</font>&nbsp;&nbsp;&nbsp;&nbsp;
                        订单状态： <c:if test="${order.order_state==0}">
                            <a style="color: red;"
                               href="${pageContext.request.contextPath}/payOrderAgain?oid=${order.order_id}">付款</a>
                        </c:if>
                        <c:if test="${order.order_state==1}">
                            已付款即将发货
                        </c:if>
                        <c:if test="${order.order_state==2}">已发货，
                            <a style="color: red;"
                               href="${pageContext.request.contextPath}/updateUserState?oid=${order.order_id}">确认收货</a>
                        </c:if>
                ...
                <c:forEach items="${order.oiList}" var="orderItem">
                    <tr>
                        <td width="60"><img
                                src="${pageContext.request.contextPath}/${orderItem.product.product_image}" />
                        </td>
                        <td>${orderItem.product.product_name}</td>
                        <td>${orderItem.product.product_specialprice}</td>
                        <td class="quantity" width="60">
                                ${orderItem.orderitem_count}
                        </td>
                        <td width="140">
								    <span class="subtotal">
								    	￥ ${orderItem.orderitem_unitprice}
									</span>
                        </td>
                    </tr>
                </c:forEach>
